...
async function fetchData() {
  try {
    ...
    data.map((item: { completed: boolean; }) => item.completed ? countCompleted.value++ : "")
  } catch {
...
const handleUpdateItems = (updatedItems: Item[]) => {
  ...
  items.value.map((item: { completed: boolean; }) => item.completed ? countCompleted.value++ : "")
...
<template>
  <section class="flex justify-center items-center min-h-[100vh]">
    <div class="w-[700px] m-auto rounded-[5px] shadow-md border-t-[5px] border-solid border-black relative">
      <Header />
      <ListTodo 
