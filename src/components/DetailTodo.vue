...
const handleMoveUp = (index:number) =>{
    if (index > 0) {
        [props.items[index], props.items[index - 1]] = [props.items[index - 1], props.items[index]];
...
<template>
  <div
    v-if="isActivePopup"
    class="absolute inset-x-0 m-auto max-w-[400px] top-[-50px] shadow-md z-10 bg-white"
  >
    ...
    <ul class="border-y border-solid border-[#b6b5b5]">
      <li
        :class="[customStyle, index == 0 ? 'opacity-30 cursor-not-allowed hover:bg-white' : '']"
        @click="handleMoveUp(index)"
      >
        Move up
      </li>
      <li
        :class="[customStyle, index == items.length - 1 ? 'opacity-30 cursor-not-allowed hover:bg-white' : '']"
      ...
      <li
        :class="customStyle"
        @click="handleDescription(detailsTodo.id)"
      >
    ...
    <div class="flex items-center p-[20px] justify-between">
      <h3
        ...
        @click="handleDelete(index,detailsTodo.id)"
      >
        Delete Task
      </h3>
    </div>
